estimate_bandwidth_silent <- function (data, method = "silverman", value = NULL) 
{
  data <- na.omit(data)
  npoints = nrow(data)
  ndim <- ncol(data)
  ...
  if (method == "fixed") {
    ...
    bandwidth_final = (4/(ndim + 2))^(1/(ndim + 4)) * npoints^(-1/(ndim + 
                                                                     4)) * stdev
  }
